...
CipStatus NetworkHandlerInitialize(void)
{
    ...
    struct sockaddr_in my_address = {.sin_family = AF_INET, .sin_port = htons(kOpenerEthernetPort), .sin_addr.s_addr = interface_configuration_.ip_address };
...
CipStatus SendUdpData(struct sockaddr_in* address, int socket, CipUsint* data,
    CipUint data_length)
{
...
CipStatus HandleDataOnTcpSocket(int socket)
{
   ...
   * method which will inform us if more data is available in the socket
   ...
   fit*/
    ...
    CipUsint* read_buffer = &g_ethernet_communication_buffer[2]; /* at this place EIP stores the data length */
